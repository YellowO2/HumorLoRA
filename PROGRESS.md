...
| 2026-06-26 | Crowd Score critique (for paper): Goes et al.'s "crowd" is not a real crowd — it is the same LLM called 4 times with different role-play instructions. No actual diversity in opinions, no sampling variance, nothing that reflects real crowd distribution. More accurately described as decomposing a joke along 4 humor style dimensions and checking which apply. Deeper flaw: the 4 HSQ types (Martin et al. 2003) describe people's humor HABITS, not dimensions along which a joke can be analyzed — category error. The model's underlying humor knowledge does not change between the 4 calls; only the output token distribution shifts slightly. Prediction: will score ~55% on HaHa pairwise, same as zero-shot, confirming no added signal. Running eval_crowd_score.py to verify empirically. |
