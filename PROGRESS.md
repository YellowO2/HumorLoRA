...
See `paper/references.md` for the full annotated reference list.
...
| 2026-06-26 | Crowd Score (Goes et al. 2022) empirical result: ~53% on HaHa pairwise (n=1400 before interrupt, converged, ~20% tie rate). Essentially random — worse than zero-shot 55.4%. Confirmed our critique: 4 persona calls on the same LLM add no signal. The "crowd" is degenerate — model outputs nearly identical humor judgments regardless of personality instruction. Result logged as partial run; full 2000-pair result would converge to same value. For paper: "Crowd Score produces ~53% on HaHa pairwise, indistinguishable from random, confirming that persona-based LLM voting adds no humor judgment signal over zero-shot." |
